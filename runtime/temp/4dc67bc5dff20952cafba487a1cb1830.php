<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:50:"template/adminblue\System\shopAdvPositionList.html";i:1515044332;s:28:"template/adminblue\base.html";i:1515044332;s:45:"template/adminblue\controlCommonVariable.html";i:1515044332;s:32:"template/adminblue\urlModel.html";i:1515044332;s:34:"template/adminblue\pageCommon.html";i:1515044332;s:34:"template/adminblue\openDialog.html";i:1515044332;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
...
<table class="mytable">
	<tr>
		<td style="text-align:left;"><button style="margin:0 5px 0 0 !important;" class="btn-common btn-small" onclick="location.href='<?php echo __URL('ADMIN_MAIN/system/addshopadvposition'); ?>';">添加广告位</button></td>
		<td style="line-height:33px;">
			广告位名称：<input id="ap_name" class="input-medium input-common" type="text" placeholder="请输入广告位名称" >
			广告展示方式：
			<select id="ap_display" class="select-common">
...
function LoadingInfo(page_index) {
	...
	var type = "<?php echo $terminal; ?>";
	var ap_display = $("#ap_display").val();
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/system/shopadvpositionlist'); ?>",
		data : {
		...
		success : function(data) {
			...
			if (data['data'].length > 0) {
				for (var i = 0; i < data['data'].length; i++) {
					...
					html += '<td><input type="checkbox" name="sub" value="'+ ap_id +'"></td>';
					...
					html += '<td class="center"><a href="'+__URL('ADMIN_MAIN/system/shopadvlist?ap_id=' + ap_id)+'"> 管理广告</a> &nbsp;';
				...
				html += '<tr><td colspan="8">暂无符合条件的数据记录</td></tr>';
			}
			$("#list").html(html);
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				...
				$.ajax({
					type : "post",
					url : "<?php echo __URL('ADMIN_MAIN/System/delPlatfromAdvPosition'); ?>",
					data : { "ap_id" : ap_id },
					success : function(data) {
						if(data["code"] > 0 ){
							showMessage('success', data["message"],"<?php echo __URL('ADMIN_MAIN/System/shopAdvPositionList'); ?>");
						}else{
...
function setPlatformAdvPositionUse(eve,ap_id,is_use,is_del){
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/System/setPlatformAdvPositionUse'); ?>",
		data : {
			"ap_id" : ap_id,
			"is_use" : is_use
		},
